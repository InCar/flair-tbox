...
public class SourceORA implements ISource2017 {
    ...
    public void fetch(String vin, LocalDate date, IDataWalk dataWalk){
        ...
        if("OLD-IP24MCE".equals(str)){
            sql = "select * from TB_MNT_SIGNAL_DATA_IP24MCE_HIS t where t.VIN = '" + vin + "' and to_char(t.DATA_DATE,'yyyy-MM-dd') = '" + date.format(s_fmt) + "' order by t.DATA_DATE";
            ...
            sql = "select * from TB_MNT_SIGNAL_DATA_IP24_HIS t where t.VIN = '" + vin + "' and to_char(t.DATA_DATE,'yyyy-MM-dd') = '" + date.format(s_fmt) + "' order by t.DATA_DATE";
            ...
            sql = "select * from TB_MNT_SIGNAL_DATA_BP34_HIS t where t.VIN = '" + vin + "' and to_char(t.DATA_DATE,'yyyy-MM-dd') = '" + date.format(s_fmt) + "' order by t.DATA_DATE";
