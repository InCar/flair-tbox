 ...
 * @Author GuoKun
 * @Despriction: 07Alarm
 * @Date Created in 2018/9/5 17:03
 */
public class IP34x07Alarm extends MongoX implements IMongoX07Alarm {
    @Override
    ...
    private static byte calcTotalVolLowestl(int vehBMSPackVolV, float vehBMSPackVol) {
        /*
        IF vehBMSPackVolV=0 && vheBMSPackVol<=N
        THEN Bit3=1
        ELSE Bit3=0
    ...
    private static byte calcVolPlusBiggerl(int vehBMSCellMaxVolV, int vehBMSCellMinVolV,
        ...
        连续2S满足才报警
        */
        if (vehBMSCellMaxVolV == 0 && vehBMSCellMinVolV == 0 && (vehBMSCellMaxVol - vehBMSCellMinVol >= 0.3)) {
            return 0x01;
        }
        return 0x00;
    }
    ...
    private static byte calcIcuBrakeSysErr(int vehBrkFludLvlLow, int vehBrkSysRedBrkTlltReq,
        ...
        */
        if ((vehBrkFludLvlLow ==1 || vehBrkSysRedBrkTlltReq ==1
                || vehABSF ==1 || vehVSESts ==2 ) && vehIbstrWrnngIO == 2 ) {
            return 0x01;
        }
        return 0x00;
    }
