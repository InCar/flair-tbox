 ...
 */
public class BP34x02Motor extends MongoX implements IMongoX02Motor {
    ...
    private static Motor getMotor(Document bsonDoc, String code) {
        ...
        // 电机控制器输入电压
        float volv = parseFloatWithDef(bsonDoc, "InvtrVolV");
        if (volv == 1) {
            m.setControllerInputVoltage(0xFFFF);
        } else {
            m.setControllerInputVoltage(parseFloatWithDef(bsonDoc, "InvtrVol"));
        }
